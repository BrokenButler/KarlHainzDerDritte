...
module.exports = {
    ...
    execute(client, message, args) {
        ...
        if (args.join('').toLowerCase() === 'queue') {
            if (queue.repeatMode === 1) return message.channel.send(`You must first disable the current music in the loop mode (${client.config.app.prefix}loop) ${message.author}... try again ? ❌`);
            ...
            if (queue.repeatMode === 2) return message.channel.send(`You must first disable the current queue in the loop mode (${client.config.app.prefix}loop queue) ${message.author}... try again ? ❌`);
